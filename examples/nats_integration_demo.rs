...
use cim_domain_nix::events::{FlakeCreated, NixDomainEvent};
use cim_domain_nix::nats::{
    CommandHandler, CommandSubscriber, EventPublisher, HealthService, NatsClient, NatsConfig,
    ServiceDiscovery, ServiceInfo, SubjectMapper,
};
...
impl CommandHandler for DemoCommandHandler {
    ...
    ) -> cim_domain_nix::nats::Result<Vec<Box<dyn std::any::Any + Send>>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut discovery = ServiceDiscovery::new(client.client().clone(), service_info);
    ...
    let command_subscriber =
        CommandSubscriber::new(client.client().clone(), handler, publisher.clone());
    ...
    println!(
        "\nCommand Subjects ({}):",
        SubjectMapper::all_command_subjects().len()
    );
    ...
    println!(
        "  ... and {} more",
        SubjectMapper::all_command_subjects().len() - 5
    );

    println!(
        "\nEvent Subjects ({}):",
        SubjectMapper::all_event_subjects().len()
    );
    ...
    println!(
        "  ... and {} more",
        SubjectMapper::all_event_subjects().len() - 5
    );
