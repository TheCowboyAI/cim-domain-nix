...
use cim_domain_nix::nats::{
    NatsClient, NatsConfig, EventPublisher, CommandSubscriber,
    NixSubject, SubjectMapper, CommandAction, EventAction,
    CommandHandler, EventHandler,
};
use cim_domain_nix::commands::NixCommand;
use cim_domain_nix::events::NixDomainEvent;
use cim_domain_nix::value_objects::{MessageIdentity, CorrelationId};
...
impl DistributedEntityRegistry {
    async fn new(client: async_nats::Client) -> Self {
        ...
        let mut sub = registry.client.subscribe("entity.registry.>").await.unwrap();
    ...
    async fn register_entity(&self, metadata: EntityMetadata) -> Result<(), Box<dyn std::error::Error>> {
        // Store locally
        self.local_cache.write().await.insert(metadata.entity_id, metadata.clone());
    ...
    async fn execute_current_step(
        ...
        if let Some(workflow) = workflows.get(&workflow_id) {
            if workflow.current_step < workflow.steps.len() {
                ...
                headers.insert("X-Correlation-ID", workflow_id.to_string());
                headers.insert("X-Workflow-Name", workflow.name.clone());
                headers.insert("X-Workflow-Step", step.name.clone());
                ...
                info!("Executed workflow step '{}' with subject {}", step.name, step.subject);
            }
        }
...
impl MeshService {
    ...
    async fn start(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        tokio::spawn(async move {
            while let Some(msg) = workflow_sub.next().await {
                if msg.subject.ends_with(".completed") {
                    ...
                    if let Some(header) = msg.headers.as_ref().and_then(|h| h.get("X-Correlation-ID")) {
                        if let Ok(workflow_id) = header.as_str().parse::<Uuid>() {
                            let _ = orchestrator.advance_workflow(CorrelationId(workflow_id)).await;
                        }
                    }
                }
            }
        ...
        let mut health_sub = self.client.subscribe(format!("health.{}", service_id)).await?;
        ...
        tokio::spawn(async move {
            while let Some(msg) = health_sub.next().await {
                ...
                if let Some(reply) = msg.reply {
                    let _ = msg.respond(serde_json::to_vec(&health).unwrap()).await;
                }
            }
        });
...
async fn demo_distributed_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let workflow_id = service_a.orchestrator.start_workflow(
    ...
    for service in [&service_a, &service_b, &service_c] {
        let entities = service.coordinator.registry.query_by_correlation(workflow_id).await;
        println!("  Service {}: {} entities", service.service_id, entities.len());
    }
