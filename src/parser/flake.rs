...
impl FlakeParser {
    ...
    fn find_flake_attrset(ast: &SyntaxNode) -> Result<SyntaxNode> {
        ...
        ast.children()
            ...
            .ok_or_else(|| NixDomainError::ParseError("Not a valid flake: no attribute set found".into()))
    }

    fn extract_description(attrset: &SyntaxNode) -> Result<Option<String>> {
        Ok(get_attribute_value(attrset, "description")
    ...
    fn extract_input_url(node: &SyntaxNode) -> Option<String> {
        ...
        if node.kind() == SyntaxKind::NODE_STRING {
            ...
            get_attribute_value(node, "url")
                .and_then(|url_node| extract_string_value(&url_node))
        } else {
            None
        }
    }
