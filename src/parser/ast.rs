...
pub fn find_attribute(attrset: &SyntaxNode, name: &str) -> Option<SyntaxNode> {
    ...
    attr_set_node.children().find(|child| {
...
pub fn from_syntax_node(node: &SyntaxNode) -> Result<NixAst, AstError> {
    match node.kind() {
        ...
        SyntaxKind::NODE_INHERIT => {
            ...
            Err(AstError::InvalidStructure(
                "Inherit must be inside an attribute set".to_string(),
            ))
        }
        ...
        _ => {
            ...
            match text.as_str() {
                ...
                _ => {
                    ...
                    Err(AstError::UnsupportedNode(format!(
...
fn parse_literal(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if let Some(token) = node.first_token() {
        match token.kind() {
            ...
            _ => {
                ...
                match text.as_str() {
                    ...
                    "false" => Ok(NixAst::Bool(false)),
                    "null" => Ok(NixAst::Null),
                    _ => Err(AstError::UnsupportedNode(format!(
                        "Unknown literal: {:?}",
                        node.kind()
                    ))),
                }
        ...
        Err(AstError::InvalidStructure(
            "Literal node has no tokens".to_string(),
        ))
    }
}
...
fn parse_function(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.is_empty() {
        return Err(AstError::InvalidStructure(
            "Function has no children".to_string(),
        ));
    }
    ...
    let param = match children[0].kind() {
        ...
        SyntaxKind::NODE_PATTERN => parse_pattern(&children[0])?,
        _ => {
            return Err(AstError::InvalidStructure(
...
fn parse_pattern_field(node: &SyntaxNode) -> Result<PatternField, AstError> {
    ...
    if children.is_empty() {
        return Err(AstError::InvalidStructure(
            "Pattern field has no children".to_string(),
        ));
    }
    ...
    let name = match children[0].kind() {
        ...
        _ => {
            return Err(AstError::InvalidStructure(
                "Pattern field missing name".to_string(),
            ))
        }
    };
...
fn parse_apply(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.len() < 2 {
        return Err(AstError::InvalidStructure(
            "Apply needs at least 2 children".to_string(),
        ));
    }
...
fn parse_let(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    let body =
        body.ok_or_else(|| AstError::InvalidStructure("Let expression missing body".to_string()))?;
...
fn parse_if(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.len() < 3 {
        return Err(AstError::InvalidStructure(
            "If expression needs condition, then, and else parts".to_string(),
        ));
    }
...
fn parse_with(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.len() < 2 {
        return Err(AstError::InvalidStructure(
            "With expression needs namespace and body".to_string(),
        ));
    }
...
fn parse_assert(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.len() < 2 {
        return Err(AstError::InvalidStructure(
            "Assert expression needs condition and body".to_string(),
        ));
    }
...
fn parse_binary_op(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.len() < 2 {
        return Err(AstError::InvalidStructure(
            "Binary operation needs at least 2 operands".to_string(),
        ));
    }
    ...
    let op = op.ok_or_else(|| {
        AstError::InvalidStructure("Binary operation missing operator".to_string())
    })?;
...
fn parse_unary_op(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.is_empty() {
        return Err(AstError::InvalidStructure(
            "Unary operation needs an operand".to_string(),
        ));
    }
    ...
    let op = op.ok_or_else(|| {
        AstError::InvalidStructure("Unary operation missing operator".to_string())
    })?;
...
fn parse_select(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.len() < 2 {
        return Err(AstError::InvalidStructure(
            "Select needs expression and attribute path".to_string(),
        ));
    }
...
fn parse_has_attr(node: &SyntaxNode) -> Result<NixAst, AstError> {
    ...
    if children.len() < 2 {
        return Err(AstError::InvalidStructure(
            "Has attr needs expression and attribute path".to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_create_attrset() {
        let attrs = vec![("name", "\"my-package\""), ("version", "\"1.0.0\"")];
