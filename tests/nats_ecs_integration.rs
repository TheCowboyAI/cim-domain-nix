...
#[cfg(test)]
mod tests {
    use cim_domain_nix::nats::{CommandAction, EventAction, NixSubject, SubjectMapper};
    ...
    fn test_correlation_filtering() {
        ...
        let workflow1_count = workflow1_entities
            .iter()
            .filter(|(_, corr)| *corr == correlation1)
            .count();
        assert_eq!(workflow1_count, 2);

        let workflow2_count = workflow2_entities
            .iter()
    ...
    fn test_wildcard_system_subscriptions() {
        ...
        let flake_matches = all_commands
            .iter()
            .filter(|s| {
        ...
        let create_matches = all_commands
            .iter()
            .filter(|s| {
                let parts: Vec<&str> = s.to_string().split('.').collect();
    ...
    fn test_command_event_correspondence() {
        ...
        for (cmd_action, evt_action) in command_event_pairs {
            ...
            assert_eq!(
                cmd_parts[2], evt_parts[2],
                "Aggregate mismatch for {:?}/{:?}",
                cmd_action, evt_action
            );
        }
    }
    ...
    fn test_subject_algebra_composition() {
        ...
        let intersection: HashSet<_> = flake_commands
            .intersection(&package_commands)
            .cloned()
            .collect();
