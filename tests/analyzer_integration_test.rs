...
use cim_domain_nix::analyzer::{AnalyzerConfig, NixAnalyzer};
use std::path::Path;
...
async fn test_analyzer_detects_security_issues() {
    ...
    assert!(
        !report.security_issues.is_empty(),
        "Should find security issues"
    );

    // Check for specific issue types
    let has_insecure_fetcher = report.security_issues.iter().any(|issue| {
        matches!(
            issue.issue_type,
    ...
    let has_weak_hash = report.security_issues.iter().any(|issue| {
        matches!(
            issue.issue_type,
...
async fn test_analyzer_detects_dead_code() {
    ...
    let has_unreachable = report.dead_code.iter().any(|dead| {
        matches!(
            dead.code_type,
